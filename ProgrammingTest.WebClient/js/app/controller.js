app.controller('memberLister',
    ['$scope', 'members', function ($scope, members) {
        ...
        $scope.AddMember = function (member) {
            ...
            $scope.Busy = true;
            $scope.Members.push(member); // proactively insert the row - makes the app looks faster :)

            members.create(member).then(function (response) {
                $scope.Busy = false;
                $scope.Members.pop(); //remove and re-add the record, it will have an ID and created date now.
